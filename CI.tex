...
\title[08 - Continuous integration and delivery]{Continuous integration and delivery \\ \scriptsize{A quick guide through the wonders of teamwork with distributed version control systems, dependency management, build automation, and continuous integration and delivery.}}
...
\begin{frame}[fragile, allowframebreaks]{What could possibly go wrong? Highway to Hell }
	\begin{block}{Diverging histories}
		\begin{itemize}
			\item a sub-team must develop a new feature, picks the latest stable code
			\item work begins, and in the meantime the software changes
			\item the new feature is now complete
			\item it just does not work the current version
		\end{itemize}
		\textbf{Keep your code in sync with the main development line}
	\end{block}
	\begin{block}{It was working two minutes ago}
		\begin{itemize}
			\item a team writes a new feature and tests it
			\item they show the feature five minutes later to the project leader
			\item it does not work anymore
			\item the team can't figure out what happened, and needs to spend time debugging
			\item the leader can't see the feature, and must wait for the developers to iron the problem out
		\end{itemize}
		\textbf{Make sure you can restore any small change you made}
	\end{block}
	\begin{block}{Stylish hell}
		\begin{itemize}
			\item development styles are misaligned
			\begin{itemize}
				\item CRLF vs. LF
			...
			\end{itemize}
			\item developers' IDEs are configured differently
			\item the project continuously flips from one style to the other, inconsistently
			\item it becomes impossible to localize changes
		\end{itemize}
		\textbf{Agree on style (possibly using existing shared conventions), and enforce its adoption}
	\end{block}
	\begin{block}{Tests as decoration}
		\begin{itemize}
			\item a feature has nice tests attached
			\item the tests are never executed
			\item the tested code ends up being broken
		\end{itemize}
		\textbf{Execute your tests for every change you make}
	\end{block}
	\begin{block}{But it works on my PC}
		\begin{itemize}
			\item tests run correctly on the machine of the one who wrote them
			\item they don't on the client's though
			\item developers must understand if they need to debug the tests or debug the code
		\end{itemize}
	...
	\begin{block}{Unexpected merge failure}
		\begin{itemize}
			\item the team develops a new feature, everything seems to work
	...
	\begin{block}{Dependency hell}
		\begin{itemize}
			\item a new feature needs a library
			\item it gets imported directly in the code base
...
\begin{frame}[fragile, allowframebreaks]{How to improve? Stairway to heaven}
	...
	\begin{block}{It's delivering}
		\begin{itemize}
			...
			\item someone spends several hours per week just for packaging alpha releases
		\end{itemize}
...
\begin{frame}[fragile]{No silver bullet}
	\begin{itemize}
		\item No single magic solution to all these problems
		\item they must be tackled with the a combination of tools and practices for:
		\begin{itemize}
			\item Development
			\item Sharing
...
\begin{frame}[fragile, allowframebreaks]{GitHub}
	...
	\begin{block}{Why GitHub}
		\begin{itemize}
			...
			\item Many third-part services and open API
...
\begin{frame}[fragile]{Trick revealed}
  ...
  \begin{itemize}
    \item Jooq Joo$\lambda$
    \begin{itemize}
      \item Used for unchecked lambda functions
    \end{itemize}
    \item Apache Commons I/O
    \begin{itemize}
      \item Used for converting a resource stream pointing to a \texttt{String} 
    \end{itemize}
    \item A retrofit-based API for TheTVDB
    \begin{itemize}
      \item Queries TheTVDB given a valid API key, hiding all the HTTP communication and JSON parsing
    \end{itemize}
  \end{itemize}
...
\begin{frame}[fragile, allowframebreaks]{Let's build our toy example}
	\java{01-dependencies/src/main/java/it/unibo/ci/PrintBreakingBad.java}{\tiny}
	\groovy{01-dependencies/build.gradle.kts}{\scriptsize}
	\groovy{01-dependencies/settings.gradle.kts}{\scriptsize}
...
\begin{frame}[fragile]{Dependency management in Gradle}
	\begin{itemize}
		...
		\begin{itemize}
			\item Where to retrieve software from (e.g. Maven Central or JCenter)
			\item The list of dependencies for each scope
			\item Some scopes inherit other scopes (e.g. \texttt{runtime} by default contains the whole \texttt{implementation} scope)
		\end{itemize}
...
\begin{frame}[fragile]{Tests as part of the build}
	...
	\begin{block}{Tests and Gradle, how to:}
		\begin{itemize}
			...
			\item Configure the testing framework as needed
            \begin{itemize}
                \item Junit 4 comes pre-configured
                \item Junit 5 needs the engine to be enabled
                \item Other frameworks may require plugins / configuration / libraries
            \end{itemize}
...
\begin{frame}[fragile, allowframebreaks]{An example with Scala and JUnit}
	\scala{04-junit/src/main/scala/it/unibo/test/Test.scala}{\normalsize}
	\scala{04-junit/src/test/scala/MyTest.scala}{\tiny}
	\groovy{04-junit/build.gradle.kts}{\tiny}
\end{frame}

\begin{frame}[fragile, allowframebreaks]{An example with ScalaTest and Scoverage}
...
\begin{frame}[fragile]{Travis CI}
	\begin{itemize}
		...
		\item Cronjobs
		\item Multi-platform builds (Windows, MacOS, Linux)
		\item Multi-arch builds (amd64, arm64...)
...
% 	\end{itemize}
% 	\begin{block}{In \texttt{build.gradle.kts}}
